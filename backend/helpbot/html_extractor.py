...
class HTMLExtractor:
    ...
    def find_best_match(self, user_query: str, entries: List[Dict[str, str]]) -> Optional[Dict[str, str]]:
        """Finds the best matching error entry based on keywords and exact ID."""
        if not entries:
            return None

        user_query_lower = user_query.lower().strip()

        # Prioritize exact "Error Log #<number>" matches
        ...
        best_match = None
        highest_score = 0
        query_words = set(re.findall(r'\b\w{3,}\b', user_query_lower))
        ...
        for entry in entries:
            ...
            score += len(query_words.intersection(explanation_words))
        ...
        if highest_score > 0:
            logger.info(f"Found best match by keyword with score {highest_score}: {best_match.get('error_code')}")
            return best_match
        
        logger.warning("No keyword match found, returning first entry as fallback.")
